...
export class OverlayController {
  ...
  __validateConfiguration(newConfig) {
    ...
    if (newConfig.isTooltip && newConfig.placementMode !== 'local') {
      throw new Error('.isTooltip should be configured with .placementMode "local"');
    }
    if (newConfig.isTooltip && !newConfig.handlesAccessibility) {
      throw new Error('.isTooltip only takes effect when .handlesAccessibility is enabled');
    }
